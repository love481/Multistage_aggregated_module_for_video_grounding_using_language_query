...
class TACoS(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        map_gt[3, :] = gt_s - np.arange(num_clips+1)  ## for each clip regress the target starting ground truth moment
        map_gt[4, :] = gt_e - np.arange(num_clips+1)  ## for each clip regress the target ending ground truth moment
