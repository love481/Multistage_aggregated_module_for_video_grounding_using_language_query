...
def bce_rescale_loss(config, logits_text, logits_visual, logits_iou, iou_mask_map, gt_maps, gt_times, word_label, word_mask,writer=None):
    T = gt_maps.shape[-1]
    ...
    s_e_time = (s_e_idx + logits_iou[:,:2,:,:]).clone().detach().to(device=s_e_idx.device)
    iou_mask_map = iou_mask_map.to(device=s_e_time.device)
    ...
    loss_value = config.W1*loss_entropy  + config.W2* loss_reg + config.W3*loss_iou + config.W4*text_loss
    #writer.writerow([float(loss_entropy),float(loss_reg),float(loss_iou), float(text_loss),float(loss_value)])
